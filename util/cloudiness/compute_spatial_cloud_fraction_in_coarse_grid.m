...
for it = 1:NT
    ...
    if sum(ncldmask_tmp,'all')==numel(ncldmask_tmp)
        ...
        for i = 1:npixel
            ...
            ccenx = cgrid.LONsub(i); 
            cceny = cgrid.LATsub(i);
            ...
            ngrid_LON_nb = ngrid.LON(rmask);
            ngrid_LAT_nb = ngrid.LAT(rmask);
            ...
            elseif sum(ncldmask_nearby)==0
                ...
                if length(lon_sel)<=6
                    ...
                    for ip = 1:length(extraPnts.lon)
                        ...
                        locmask = ismembertol(ngrid_LON_nb, extraPnts.lon(ip),1e-4) & ismembertol(ngrid_LAT_nb,extraPnts.lat(ip),1e-4);
                        box = [box, ncldmask_nearby(locmask)];
                        
                    end
                    extraPnts.cldmask = box;
