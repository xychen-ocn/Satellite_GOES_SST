...
%     dateID = tmp{end}(1:5);
    ...
    %if ~exist([datasvdir filesep blob_statFN])
        ...
        %SSTgap_thres = 0.5;  % when cloudy pixel is less than 50% of the scene, fill the SST map;
        [SST_filled, filled_flag] = fill_cloudy_pixels(SST_raw, LON, LAT, SSTgap_thres);
        ...
        if length(find(filled_flag==1))/length(filled_flag)>0.1
            ...
            pause(0.2);
            xc_savefig(gcf, figsvdir, [dateID '_warm_anom_obtain_steps.jpg'], [0 0 10 4]);
            %     %close gcf
            %
            ...
            for it =1:size(SST_maps,3)
                ...
                if filled_flag(it)
                    ...
                    if thres_cnt > 0
                        ...
                        [individual_blobs, Nblobs] = remove_doppelgangers(blob_all);
                        ...
                        if isRHB_day && isRHB_segtime
                            ...
                            plot(RHB_data.lon, RHB_data.lat, '--k');
